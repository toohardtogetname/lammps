...
PairORB::PairORB(LAMMPS *lmp) : Pair(lmp)
{
    single_enable           = 0;
    restartinfo             = 0;
    one_coeff               = 1;
    ...
    if (!PairORB::finalized)
    {
        PairORB::finalized = 1;
        std::atexit(PairORB::finalize);
    }
}
...
int PairORB::finalized = 0;

void PairORB::finalize()
{
    if(Py_IsInitialized()) Py_Finalize();
}

void PairORB::allocate()
{
    allocated = 1;
...
void PairORB::compute(int eflag, int vflag)
{
    ev_init(eflag, vflag);

    if (eflag_atom)
    {
        error->all(FLERR, "Pair style ORB does not support atomic energy");
    }

    if (vflag_atom)
    {
        error->all(FLERR, "Pair style ORB does not support atomic virial pressure");
    }
...
void PairORB::performGNN()
{
    ...
    // perform Graph Neural Network Potential of ORB
...
void PairORB::settings(int narg, char **arg)
{
    if (comm->nprocs > 1)
    {
        error->all(FLERR, "Pair style ORB does not support MPI parallelization");
    }
...
void PairORB::coeff(int narg, char **arg)
{
    int i, j;
    int count;
    ...
    if (ntypesEff < 1)
    {
        error->all(FLERR, "There are no elements for pair_coeff of ORB.");
    }
    ...
    if (this->cutoff <= 0.0)
    {
        error->all(FLERR, "Cutoff is not positive for pair_coeff of ORB.");
    }
...
double PairORB::init_one(int i, int j)
{
    if (setflag[i][j] == 0)
    {
...
void PairORB::init_style()
{
    if (strcmp(update->unit_style, "metal") != 0)
    {
        error->all(FLERR, "Pair style ORB requires 'units metal'");
    }
    ...
    {
        error->all(FLERR, "Pair style ORB requires periodic boundary condition");
    }
...
double PairORB::initializePython(const char *name, int gpu)
{
    if (this->initializedPython != 0)
    {
        return this->cutoff;
    ...
    pyName = PyUnicode_DecodeFSDefault("orb_driver");
    if (pyName != nullptr)
    {
    ...
    if (pyModule != nullptr)
    {
        pyFunc = PyObject_GetAttrString(pyModule, "orb_initialize");
        ...
        pyFunc = PyObject_GetAttrString(pyModule, "orb_get_energy_forces_stress");
    ...
    if (this->initializedPython == 0)
    {
        ...
        error->all(FLERR, "Cannot initialize python for pair_coeff of ORB.");
    }
...
double PairORB::calculatePython()
{
    int i;
    int iatom;
    int natom = list->inum;
    ...
    {
        error->all(FLERR, "Cannot calculate energy, forces and stress by python of ORB.");
    }
...
int PairORB::elementToAtomNum(const char *elem)
{
    char elem1[16];
...
void PairORB::toRealElement(char *elem)
{
    int n = strlen(elem);
    n = n > 2 ? 2 : n;
