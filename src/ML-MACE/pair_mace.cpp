...
PairMACE::PairMACE(LAMMPS *lmp) : Pair(lmp)
{
    ...
    if (!PairMACE::finalized)
    {
        PairMACE::finalized = 1;
        std::atexit(PairMACE::finalize);
    }
}
...
int PairMACE::finalized = 0;

void PairMACE::finalize()
{
...
void PairMACE::compute(int eflag, int vflag)
{
    ...
    if (eflag_atom)
    {
        error->all(FLERR, "Pair style MACE does not support atomic energy");
    }

    if (vflag_atom)
    {
        error->all(FLERR, "Pair style MACE does not support atomic virial pressure");
    }
...
void PairMACE::performGNN()
{
    ...
    // perform Graph Neural Network Potential of MACE
...
void PairMACE::settings(int narg, char **arg)
{
    if (comm->nprocs > 1)
    {
        error->all(FLERR, "Pair style MACE does not support MPI parallelization");
    }
...
void PairMACE::coeff(int narg, char **arg)
{
    ...
    if (ntypesEff < 1)
    {
        error->all(FLERR, "There are no elements for pair_coeff of MACE.");
    }
    ...
    if (this->cutoff <= 0.0)
    {
        error->all(FLERR, "Cutoff is not positive for pair_coeff of MACE.");
    }
...
void PairMACE::init_style()
{
    ...
    {
        error->all(FLERR, "Pair style MACE requires 'units metal'");
    }
    ...
    {
        error->all(FLERR, "Pair style MACE requires periodic boundary condition");
    }

    neighbor->add_request(this, NeighConst::REQ_FULL);
}

int PairMACE::withDFTD3()
{
    return 0;
}

int PairMACE::withGPU()
{
    return 0;
}

void PairMACE::finalizePython()
{
...
double PairMACE::initializePython(const char *name, int dftd3, int gpu)
{
    if (this->initializedPython != 0)
    {
        return this->cutoff;
    ...
    pyName = PyUnicode_DecodeFSDefault("mace_driver");
    if (pyName != nullptr)
    {
    ...
    if (pyModule != nullptr)
    {
        pyFunc = PyObject_GetAttrString(pyModule, "mace_initialize");
        ...
        pyFunc = PyObject_GetAttrString(pyModule, "mace_get_energy_forces_stress");
    ...
    if (this->initializedPython == 0)
    {
        ...
        error->all(FLERR, "Cannot initialize python for pair_coeff of MACE.");
    }
...
double PairMACE::calculatePython()
{
    int i;
    int iatom;
    int natom = list->inum;
    ...
    {
        error->all(FLERR, "Cannot calculate energy, forces and stress by python of MACE.");
    }
...
int PairMACE::elementToAtomNum(const char *elem)
{
    char elem1[16];
...
void PairMACE::toRealElement(char *elem)
{
    int n = strlen(elem);
    n = n > 2 ? 2 : n;
