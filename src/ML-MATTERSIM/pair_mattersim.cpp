...
PairMatterSim::PairMatterSim(LAMMPS *lmp) : Pair(lmp)
{
    single_enable           = 0;
    restartinfo             = 0;
    one_coeff               = 1;
    ...
    if (!PairMatterSim::finalized)
    {
        PairMatterSim::finalized = 1;
        std::atexit(PairMatterSim::finalize);
    }
}

PairMatterSim::~PairMatterSim()
{
    if (copymode)
    {
        return;
...
int PairMatterSim::finalized = 0;

void PairMatterSim::finalize()
{
    if(Py_IsInitialized()) Py_Finalize();
}

void PairMatterSim::allocate()
{
    allocated = 1;
...
void PairMatterSim::compute(int eflag, int vflag)
{
    ev_init(eflag, vflag);

    if (eflag_atom)
    {
        error->all(FLERR, "Pair style MatterSim does not support atomic energy");
    }

    if (vflag_atom)
    {
        error->all(FLERR, "Pair style MatterSim does not support atomic virial pressure");
    }
...
void PairMatterSim::settings(int narg, char **arg)
{
    if (comm->nprocs > 1)
    {
        error->all(FLERR, "Pair style MatterSim does not support MPI parallelization");
    }
...
void PairMatterSim::coeff(int narg, char **arg)
{
    int i, j;
    int iarg;
    int count;
    ...
    if (narg != (2 + ntypes) && narg != (3 + ntypes))
    {
    ...
    if (ntypesEff < 1)
    {
        error->all(FLERR, "There are no elements for pair_coeff of MatterSim.");
    }
    ...
    if (iarg == 2)
    {
        this->cutoff = this->initializePython(nullptr, dftd3, gpu);
    }
    else
    {
    ...
    if (this->cutoff <= 0.0)
    {
        error->all(FLERR, "Cutoff is not positive for pair_coeff of MatterSim.");
    }
...
double PairMatterSim::init_one(int i, int j)
{
    if (setflag[i][j] == 0)
    {
...
void PairMatterSim::init_style()
{
    if (strcmp(update->unit_style, "metal") != 0)
    {
        error->all(FLERR, "Pair style MatterSim requires 'units metal'");
    }
    ...
    {
        error->all(FLERR, "Pair style MatterSim requires periodic boundary condition");
    }

    neighbor->add_request(this, NeighConst::REQ_FULL);
}

int PairMatterSim::withDFTD3()
{
    return 0;
}

int PairMatterSim::withGPU()
{
    return 0;
}

void PairMatterSim::finalizePython()
{
...
double PairMatterSim::initializePython(const char *name, int dftd3, int gpu)
{
    if (this->initializedPython != 0)
    {
        return this->cutoff;
    ...
    pyName = PyUnicode_DecodeFSDefault("mattersim_driver");
    if (pyName != nullptr)
    {
    ...
    if (pyModule != nullptr)
    {
        pyFunc = PyObject_GetAttrString(pyModule, "mattersim_initialize");

        if (pyFunc != nullptr && PyCallable_Check(pyFunc))
        {
            pyArg1 = name == nullptr ? Py_None : PyUnicode_FromString(name);
        ...
        pyFunc = PyObject_GetAttrString(pyModule, "mattersim_get_energy_forces_stress");
    ...
    if (this->initializedPython == 0)
    {
        ...
        error->all(FLERR, "Cannot initialize python for pair_coeff of MatterSim.");
    }
...
double PairMatterSim::calculatePython()
{
    int i;
    int iatom;
    int natom = list->inum;
    ...
    {
        error->all(FLERR, "Cannot calculate energy, forces and stress by python of MatterSim.");
    }
...
int PairMatterSim::elementToAtomNum(const char *elem)
{
    char elem1[16];
...
void PairMatterSim::toRealElement(char *elem)
{
    int n = strlen(elem);
    n = n > 2 ? 2 : n;
